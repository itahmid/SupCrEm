...
###############################################################################
# Skins/ControlPanel/RemoveStaff.pm.pl -> RemoveStaff skin module             #
###############################################################################
# DON'T EDIT BELOW UNLESS YOU KNOW WHAT YOU'RE DOING!                         #
###############################################################################
package Skin::CP::RemoveStaff;
...
sub show {
  ...
  $Body .= $Dialog->LargeHeader(title => "Remove Staff Accounts", colspan => 3);
  ...
  if (scalar(@{ $in{'input'}->{'STAFF_ACCOUNTS'} }) == 1) {
    $Script .= qq~    document.form.USERNAME.checked = true;\n~;
  } elsif (scalar(@{ $in{'input'}->{'STAFF_ACCOUNTS'} }) > 1) {
    $Script .= qq~    for (var h = 0; h <= ~.(scalar(@{ $in{'input'}->{'STAFF_ACCOUNTS'} }) - 1).qq~; h++) {\n~;
  ...
  $Body = $Dialog->Form(
    ...
    hiddens => [
      { name => "CP", value => "1" },
      { name => "action", value => "DoRemoveStaff" },
...
sub do {
  ...
  my $Body = $Dialog->Text(text => "The selected staff accounts have been removed.");

  $Body = $Dialog->Dialog(body => $Body);
  $Body = $Dialog->SmallHeader(titles => "remove staff accounts").$Body;
