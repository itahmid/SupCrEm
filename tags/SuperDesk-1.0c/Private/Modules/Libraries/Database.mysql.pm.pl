###############################################################################
# UltraBoard 2000 Database Engine (MySQL)                                     #
...
sub Connect {
  ...
  my $DSN  = "DBI:$in{'DRIVER'}:$in{'DATABASE'}:$in{'IP'}";
     $DSN .= ":$in{'PORT'}" if $in{'PORT'};
  ...
  for (my $i = 0; $i <= $#Keys; $i++) {
    ...
    if ($Pro->[1] eq "autonumber") {
      if ($Pro->[2] <= 3) {
        ...
        $Field .= " SMALLINT($Pro->[2])";
      } elsif ($Pro->[1] <= 7) {
        $Field .= " MEDIUMINT($Pro->[2])";
      } elsif ($Pro->[1] <= 10) {
        $Field .= " INT($Pro->[2])";
      } elsif ($Pro->[1] <= 19) {
        $Field .= " BIGINT($Pro->[2])";
...
sub Update {
  ...
  my $Query = "SELECT ".join(", ", @Fields)." FROM $self->{'PREFIX'}$Table WHERE $PrimaryKey = ".$DB->quote($in{'KEY'});
  ...
  foreach my $key (keys %{ $Hash } ) {
    push (@Fields, $key."=".$DB->quote($Hash->{$key}));
  }
...
sub BinarySelect {
  ...
  $Query .= " FROM $self->{'PREFIX'}$Table WHERE $PrimaryKey=".$DB->quote($in{'KEY'});
...
sub Export {
  ...
  if ($DBTable->{$PrimaryKey}[1] eq "autonumber") {
    my $Query = "SELECT $PrimaryKey FROM $self->{'PREFIX'}$Table ORDER BY $PrimaryKey DESC LIMIT 1";
...
sub Synchronize {
  ...
  if ($Function eq "INSERT") {
    ...
    my @Fields;
    foreach my $key (keys %{ $Values } ) {
      push (@Fields, $key."=".$DBH->quote($Values->{$key}));
    }
