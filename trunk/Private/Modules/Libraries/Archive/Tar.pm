...
package Archive::Tar;
###############################################################################
# Modified by Jacky Yung of UltraScripts.com, Inc.                            #
# Used in SuperDesk by Gregory Nolle.                                         #
###############################################################################
use strict;
use Carp;
use Cwd;
...
# modified by Jacky Yung, UltraScripts.com, Inc.
# to fix cwd function do not return the path in some BSD servers.
sub _cwd {
    my @path = split(/\//, $0);
    pop @path;
    return join("/", @path);
...
sub extract {
	...
	foreach (@{$self->{'_data'}}) {
		...
		# modified by Jacky Yung, UltraScripts.com, Inc.
    # to fix cwd function do not return the path in some BSD servers.
    $current = cwd() || &_cwd() || $cwd_path || $UB::PATH;
		#$current = cwd;
		foreach (@path) {
		    if (-e $_ && ! -d $_) {
